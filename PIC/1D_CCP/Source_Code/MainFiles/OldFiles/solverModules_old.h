...
void triDiagSolver(double *phi,
  ...
  for (int i = 0; i < nn; ++i) {
    if (i == (0)) {
      if(b[i] == 0) {cout << "Warning: b[i] == 0" << endl;}
      c_prime[i] = c[i]/b[i];
      d_prime[i] = d[i]/b[i];
    } else {
      if(b[i]-a[i]*c_prime[i-1] == 0) {cout << "Warning: b[i]-a[i]*c_prime[i-1] == 0" << endl;}
...
(double *u, double *RHS, double *D,
 ...
 double dt, int n_cell, double *flux_L, double *flux_R) {
...
/*
  ...
  */
  ...
  for (int i = 0; i < n_cell; ++i) {
    if (i == 0) {
      *flux_L = -0.5*D[i]*(u[i] - n_L)/dx;
      u_new[i] = RHS[i]*dt + 
	       A*0.5*(D[i+1]+D[i])*(u[i+1] - u[i]) - 
	       A*0.5*D[i]*(u[i] - n_L) + u[i];

    } else if (i == n_cell - 1) {
      *flux_R = -0.5*D[i]*(n_R - u[i])/dx;
      u_new[i] = RHS[i]*dt + 
	       A*0.5*D[i]*(n_R - u[i]) - 
